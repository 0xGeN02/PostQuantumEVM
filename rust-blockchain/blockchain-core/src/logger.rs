...
impl BlockchainLogger {
    pub fn new() -> Self {
        ...
        let logs_dir = format!("logs/{}", session_id);
        let blocks_dir = format!("{}/blocks", logs_dir);
        ...
        if !Path::new(&blocks_dir).exists() {
            fs::create_dir_all(&blocks_dir)
                .expect("Failed to create blocks directory");
        }
        
        println!("📁 Session logs will be saved to: {}/", logs_dir);
        println!("📦 Block files will be saved to: {}/", blocks_dir);
    ...
    pub fn log_block_creation(&self, block: &Block) {
        ...
        self.write_to_subfolder("blocks", &block_filename, &serde_json::to_string_pretty(&log_entry).unwrap());
    }
    ...
    pub fn log_blockchain_state(&self, blockchain: &Blockchain) {
        ...
        self.write_to_file("blockchain_state.json", &serde_json::to_string_pretty(&blockchain_data).unwrap());
    }
    ...
    pub fn create_summary_report(&self, blockchain: &Blockchain) {
        ...
        let summary = format!(
            ...
            avg_diff,
            blockchain.is_valid(),
            blockchain.blocks.iter().map(|block| {
                format!(
        ...
        println!("📄 Summary report saved to {}/summary_report.txt", self.logs_dir);
    }
}